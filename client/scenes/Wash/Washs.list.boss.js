...
import WashsRowBoss from '../../components/washs/washs.row.boss'
...
const WashsListBoss = () => {
  ...
  const list = useSelector((s) => s.washs.list)
  const curPage = useSelector((s) => s.washs.currentPage)
  const totalPages = useSelector((s) => s.washs.numberOfPages)
  const isLoaded = useSelector((s) => s.washs.isLoaded)
  ...
  const paginate = (pageNumber) => {
    history.push(`/washboss/list/${pageNumber}`)
  }
  ...
  const onChangeCuwashmerUppercaseRussian = (e) => {
    const { name, value } = e.target
    setSearch((prevState) => ({
      ...prevState,
      [name]: value
  ...
  return (
    <div>
      ...
      <div>
        ...
        <Link to={`/washboss/create/${num ? Number(num) : ''}`}>
          <button
            type="button"
