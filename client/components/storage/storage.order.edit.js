...
const StoragesEdit = (props) => {
  ...
  const [state, setState] = useState({
    ...
    gen: props.gen,
    mod: props.mod,
    preorder: props.preorder.length !== 0 ? props.preorder : [{ storageItem: '' }],
    name: props.name,
    phone: props.phone,
    prepay: props.prepay,
  ...
  const sendData = () => {
    ...
    if (!state.phone) notify('Заполните поле Телефон')
      ...
      history.push(`/storages/edit/${props.id_storages}/${props.num ? props.num : ''}`)
                  ...
                  >
                    <td className="w-full lg:w-auto p-2 text-gray-800 text-center border border-b block table-cell relative static">
                      <input
                        ...
                        name="storageItem"
                        list="storages_list"
                        value={inputField.storageItem}
                        defaultValue={
                          state.preorder.find((it, id) => id === index)
                            ? state.preorder.find((it, id) => id === index).storageItem
                            : ''
                        }
