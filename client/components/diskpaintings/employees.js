  ...
  return (
    <div className="flex flex-col -mx-3">
      <div className="flex gap-3">
        {!hideBox && currentPlace && currentPlace.shinomontazhquantity ? (
      ...
      <SalaryPercentModal
        ...
        persistPreferences={auth?.place != null ? { placeId: auth.place, group } : null}
      />
          ...
          <table>
            {employeeList && !dateEnd
              ? employeeList
                  .filter(
                    (it) =>
                      it.role.includes('Покраска дисков') &&
                    ...
                    >
                      ...
                      <td>
                        {employees.find((it) => it.id.includes(item.id) && it?.group === group) ? (
                          ...
                          >
                            {employees.find((it) => it.id.includes(item.id)).role === 'main' ? 'Старший' : ''}
                            {employees.find((it) => it.id.includes(item.id)).role === 'second' ? 'Исполнитель' : ''}
                            {employees.find((it) => it.id.includes(item.id)).role === 'student' ? 'Студент' : ''}
