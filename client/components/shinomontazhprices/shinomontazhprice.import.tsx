...
interface IShinomontazhpriceImportProps {
  onSaved: () => void
  onCancel: () => void
}

const ShinomontazhpriceImport = ({ onSaved, onCancel }: IShinomontazhpriceImportProps) => {
  ...
  const readExcel = (file: File) => {
    const promise = new Promise<Record<string, unknown>[]>((resolve, reject) => {
      ...
      fileReader.onload = (e) => {
        ...
        resolve(fromExcelRows(data, shinomontazhExcelConfig))
      }
  ...
  const sendData = () => {
    ...
    dispatch(importShinomontazhprice(loadingData))
    ...
    <div className="flex flex-1 flex-col overflow-hidden">
      <div className="flex flex-1 flex-col overflow-y-auto px-6 py-4">
        <FieldGroup>
          ...
          <Field>
            <FieldLabel htmlFor="shinomontazhprice-file">Загрузите файл Xls или Xlsx</FieldLabel>
            <Input
              id="shinomontazhprice-file"
              type="file"
              accept=".xls,.xlsx"
              onChange={(e) => {
          ...
          <Field>
            ...
            <ul className="ml-4 list-disc space-y-1 text-sm text-muted-foreground">
              ...
              <li>
                ...
                {shinomontazhTypeList
                  .map((it: { name: string; value: string }) => it.name)
                  .join(', ')}
              </li>
              <li>
                <b className="text-foreground">Категория</b> — для легкового: категория из
                справочника кузовов (Седан, Хетчбек; Седан (штампованные диски); Кроссоверы,
                внедорожник, универсалы и микроавтобусы; RUNFLAT; Другое); для грузового и сельхоз:
                Основное, Другое, Мусорки (КЭР) или Акция
              </li>
              <li>
