...
const toFormState = (item?: IShinomontazhPrice): IFormState =>
  item
    ? {
        name: item.name,
        type: item.type,
...
interface IShinomontazhpriceFormProps {
  mode: 'create' | 'edit'
  item?: IShinomontazhPrice
  onSaved: () => void
  onCancel: () => void
}

const ShinomontazhpriceForm = ({ mode, item, onSaved, onCancel }: IShinomontazhpriceFormProps) => {
  ...
  const removeItem = () => {
    if (!item?.id) return
    dispatch(deleteShinomontazhprice(item.id))
  ...
  const submit = () => {
    ...
    if (mode === 'create') {
      dispatch(createShinomontazhprice(toPayload(state)))
      notify('Услуга добавлена')
    } else if (item?.id) {
      dispatch(updateShinomontazhprice(item.id, toPayload(state)))
  ...
  const categoryOptions = isFleetType ? shinomontazhFleetCategoryList : shinomontazhCategoryList
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field data-invalid={!!errors.category}>
              ...
              >
                ...
                <SelectContent>
                  {categoryOptions.map((it: { name: string; value: string }) => (
                    <SelectItem key={it.value} value={it.value}>
                      {it.name}
                    </SelectItem>
                  ))}
...
export default ShinomontazhpriceForm
