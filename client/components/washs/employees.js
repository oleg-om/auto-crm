...
const Employee = ({
  ...
  return (
    <div className="flex flex-col -mx-3">
      {currentPlace && currentPlace.washboxes ? (
        <div className="px-3 mb-3   w-full">
          ...
          <div className="flex flex-wrap w-full relative">
            {Array.from(
              Array(currentPlace && currentPlace.washboxes ? currentPlace.washboxes : 0).keys()
            )
              .map((a) => a + 1)
              .map((it) => (
                <button
          ...
          <table>
            {employeeList && !dateEnd
              ? employeeList
                  .filter(
                    (it) =>
                      it.role.includes('Работник автомойки') && it.address.includes(auth.place)
                  )
                  .map((item) => (
                    <tr
