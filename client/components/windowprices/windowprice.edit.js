...
import { categoryList } from './windowprice.create'
...
const WashpriceUpdate = (props) => {
  ...
  const removeWashprice = (e) => {
    props.deleteFunc(props.id, e.target.value)
    history.push(`/${currentLocation}/list/${props.type}`)
  ...
  const changeWashprice = () => {
    if (!state.name) notify('Поле Название пустое')
      ...
      props.updateFunc(props.id, state)
      history.push(`/${currentLocation}/list/${props.type}`)
  ...
  return (
    <div>
      ...
      <div className=" flex my-2">
        <Link
          to={`/${currentLocation}/list/${props.type}`}
