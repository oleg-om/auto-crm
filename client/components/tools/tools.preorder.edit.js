...
const ToolUpdate = (props) => {
  ...
  const [inputFields, setInputFields] = useState(
    state.order.map((it) => ({
      toolItem: it.toolItem ? it.toolItem : '',
              ...
              <div className="-mx-3 md:flex mb-2">
                <div className="overflow-x-auto md:w-auto px-3 mb-6 md:mb-0">
                  {props.preorder.map((it) => (
                    <p key={it.toolItem}>
                      {it.toolItem} {it.quantity ? `- ${it.quantity} шт` : null}
                    </p>
                  ))}
                </div>
              </div>
                        ...
                        <input
                          ...
                          defaultValue={
                            state.order.find((it, id) => id === index)
                              ? state.order.find((it, id) => id === index).toolItem
                              : ''
                          }
