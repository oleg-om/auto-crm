...
const EmployeeJournal = () => {
  ...
  const handleCompleteDuty = async (dutyId, uniqueKey) => {
    if (!currentEmployee || !currentEmployee.positionId) {
      notify('Должность не назначена')
      return
    }
  ...
  return (
    <div>
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-6">
          <label htmlFor="journal-date" className="block text-sm font-medium text-gray-700 mb-2">
            Дата
          </label>
          <input
            id="journal-date"
            ...
            className="px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-main-600"
          />
        </div>

        <div className="mb-4">
          <h2 className="text-xl font-bold mb-2">
            {currentPosition.name} - {currentEmployee.name} {currentEmployee.surname}
          </h2>
        </div>
                  ...
                  addedDuties.map((duty) => {
                    ...
                    return (
                      <DutyEntryForm
                        ...
                        onComplete={() => handleCompleteDuty(duty._id, duty.uniqueKey)}
      ...
      <Modal
        ...
        message={
          dutyToDelete
            ? `Вы уверены, что хотите удалить обязанность "${dutyToDelete.name}" из списка? Заполненные данные будут сохранены, но обязанность больше не будет отображаться в списке.`
  ...
  return (
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex-1">
          ...
          {entry?.startTime && (
            <p className="text-sm text-gray-600 mt-1">
              Начало: <span className="font-medium">{formatTime(entry.startTime)}</span>
            </p>
          )}
          {entry?.endTime && (
            <p className="text-sm text-gray-600 mt-1">
              Окончание: <span className="font-medium">{formatTime(entry.endTime)}</span>
            </p>
          )}
        </div>
        ...
        <div className="text-gray-600">
          {entry ? (
            <div>
              {duty.isQuantitative ? (
                <p>
                  <strong>Количество:</strong> {entry.value || 'Не указано'}
                </p>
              ) : (
                <p>
                  <strong>Статус:</strong>{' '}
                  {entry.value === true || entry.value === 'true' ? 'Выполнено' : 'Не выполнено'}
                </p>
              )}
              {entry.comment && (
                <p className="mt-2">
